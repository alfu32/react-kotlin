...
fn expand_tabs(line string) string {
	...
	for ch in line.runes() {
		if ch == `\t` {
			mut spaces := editor.tab_stop - (column % editor.tab_stop)
			if spaces <= 0 {
				spaces = editor.tab_stop
			}
...
pub fn visual_column(line string, column int) int {
	...
	for ch in line.runes() {
		...
		if ch == `\t` {
			mut spaces := editor.tab_stop - (visual % editor.tab_stop)
			if spaces <= 0 {
				spaces = editor.tab_stop
			}
			visual += spaces
...
pub fn actual_column(line string, visual int) int {
	...
	for ch in line.runes() {
		mut width := 1
		if ch == `\t` {
			mut spaces := editor.tab_stop - (current % editor.tab_stop)
			if spaces <= 0 {
				spaces = editor.tab_stop
			}
			width = spaces
...
fn (b TextBuffer) gutter_text(line_idx int) string {
	...
	if num.len > editor.default_gutter_width {
		num = num[num.len - editor.default_gutter_width..]
	} else if num.len < editor.default_gutter_width {
		num = ' '.repeat(editor.default_gutter_width - num.len) + num
	}
	return '${num}  '
}
